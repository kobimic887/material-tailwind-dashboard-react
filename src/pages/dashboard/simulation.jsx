...
import {
  Card, CardHeader, CardBody, Typography, Input, Button, Tabs, TabsHeader, TabsBody, Tab, TabPanel, Spinner,
  ...
  async function predictSensitivity() {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/predict`, {
        ...
        // Do NOT set credentials, do NOT set extra headers!
        // mode: "cors" is default and can be omitted
      });
      if (!response.ok) {
        // Try to parse error, else fallback to status
        let msg = "";
        try {
      ...
      <Card>
        ...
        <CardBody className="px-6 pb-6">
          <Tabs value={activeTab}>
            <TabsHeader>
              <Tab value="predictor" onClick={() => setActiveTab("predictor")}>
                Predictor
              </Tab>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && predictSensitivity()}
                    />
                  </div>
                  ...
                  {error && (
                    <Typography variant="small" color="red" className="block">
                      {error}
                    </Typography>
                  )}
                  {result && (
